...
class MonitorBabay:
    def __init__(self):
        ...
        # self.face_model = cv2.face.FisherFaceRecognizer_create()

        # 人脸68个关键点
        self.landmark_predictor = dlib.shape_predictor("asserts/shape_predictor_68_face_landmarks.dat")
    ...
    # save_num：存储总量
    # video_path: 视频文件的路径
    def collectFacesFromVideo(self, label_index, save_interval, save_num, video_path):
        ...
        while True:
            ...
            frame = self.utils.cv2AddChineseText(frame, "帧率： " + str(int(fps_text)), (10, 30),
                                                textColor=(0, 255, 0), textSize=50)
            frame = cv2.resize(frame, (int(width) // 2, int(height) // 2))
            # cv2.imshow('Collect data', frame)
            # if cv2.waitKey(5) & 0xFF == 27:
            #     break
            # cv2.imwrite('cache/collect' + str(cTime) + '.jpg', frame)
        cap.release()
    ...
    def getEyePixelDist(self):
        ...
        while True:
            ...
            # if cv2.waitKey(5) & 0xFF == 27:
            #     break
            cv2.imwrite('cache/eye_dist' + str(cTime) + '.jpg',frame)
        cap.release()
    ...
    def run_video(self, display, video_path):
        model_path = "./face_model/model.yml"
        if not os.path.exists(model_path):
            print('人脸识别模型文件不存在，请先采集训练')
            exit()
        ...
        video_writer = cv2.VideoWriter('./record_video/out'+str(time.time())+'.avi', cv2.VideoWriter_fourcc(*'XVID'), fps, (width, height))
        import tqdm
        ...
        #         break  # Break the loop if the video ends
            ...
            # size of face in the image is too small to detect face
            ...
            frame = cv2.resize(frame, (int(width) // 2, int(height) // 2))
            # cv2.imshow('Baby watching TV', frame)
...
parser.add_argument("--video_path", 
                    type=str, 
                    default='/public/home/huangshisheng/gaussian-splatting/endingDesign/data_raw/level1/数学与应用数学05号_湖北师范大学_潘慈忠.wmv', 
                    help="视频文件路径")                     
...
if mode == 'collect':
    ...
    if args.label_id and args.img_count and args.img_interval:
        if args.video_path:
            m.collectFacesFromVideo(args.label_id,args.img_interval,args.img_count,args.video_path)
        else:
...
if mode == 'run':
    ...
    m.run_video(args.display, args.video_path)